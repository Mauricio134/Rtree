...
void RTree::LEAF(Node * node){
	for(int i = 0; i < 4; i++){
		if(!node) return;
		if(node->m_level == 0){
			for(int j = 0; j < 4; j++){
				if(node->m_branch[j].m_data.size() > 0){
					cout << "HOJA: ";cout << node->m_branch[j].m_rect.m_min[0] << "," << node->m_branch[j].m_rect.m_min[1] << " " << node->m_branch[j].m_rect.m_max[0] << "," << node->m_branch[j].m_rect.m_max[1] << endl;
		...
		this->LEAF(node->m_branch[i].m_child);
		cout << "PADRE: ";
		cout << node->m_branch[i].m_rect.m_min[0] << "," << node->m_branch[i].m_rect.m_min[1] << " " << node->m_branch[i].m_rect.m_max[0] << "," << node->m_branch[i].m_rect.m_max[1] << endl;
	}
